...
export const generateReceiptPDF = async (data, role = "client") => {
  ...
  if (data.method === "delivery" && data.address) {
    ...
    y += 16;
    doc.text(`Delivery Date: ${new Date(data.delivery_date).toLocaleDateString()}`, margin, y);
    y += 16;
    ...
    if (data.delivery_date) {
      doc.text(`Pickup Date: ${new Date(data.delivery_date).toLocaleDateString()}`, margin, y);
      y += 16;
    }
  ...
  doc.text(`Tax Included (${taxPercent}%): $${taxIncluded.toFixed(2)}`, margin + 250, y);
